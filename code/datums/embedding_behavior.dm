...
/datum/embedding_behavior
	...
	var/embedded_unsafe_removal_time
	///if we don't give a damn about EMBED_THROWSPEED_THRESHOLD
	var/embedded_ignore_throwspeed_threshold
	var/embedded_bloodloss
	///if we make the limb retracted when embedded
	var/retract_limbs
	///if we make the limb clamped when embedded
	var/clamp_limbs
