...
/atom/movable/screen/cmode/Click(location, control, params)
	...
	if(isliving(usr))
		...
		if(modifiers["right"])
			...
			if(L.mob_timers["complytoggle"]) // I am fed up with trying to triage issues with new middle click code. Here, have hacky workaround. - Zoktiik
				if(world.time > (L.mob_timers["complytoggle"] + 0.5 SECONDS))
					return
			L.mob_timers["complytoggle"] = world.time
			L.playsound_local(L, 'sound/misc/click.ogg', 100)
			L.toggle_compliance()
